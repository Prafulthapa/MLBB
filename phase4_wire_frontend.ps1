# ============================================================
#  MLBB TopUp Nepal - Phase 4: Wire Frontend to Real API
#  Run from: D:\MLBB>  .\phase4_wire_frontend.ps1
# ============================================================

Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host "  MLBB TopUp Nepal - Phase 4: Frontend + API" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""

Write-Host "[1/2] Writing wired mlbb-topup.html..." -ForegroundColor Yellow
...
.hero{position:relative;z-index:1;text-align:center;padding:48px 20px 36px;border-bottom:1px solid var(--border);background:linear-gradient(180deg,rgba(0,71,255,0.06) 0%,transparent 100%);}
...
.pay-logo{width:52px;height:52px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0;}
.pay-esewa{background:linear-gradient(135deg,#60B246,#3d8a2e);}
.pay-bank{background:linear-gradient(135deg,#1a73e8,#0d47a1);}
...
.order-btn{background:linear-gradient(135deg,#FFD700,#FF6B00);border:none;border-radius:50px;padding:16px 48px;font-family:'Orbitron',sans-serif;font-weight:900;font-size:1rem;color:#000;cursor:pointer;letter-spacing:2px;text-transform:uppercase;transition:all 0.3s;box-shadow:0 4px 30px rgba(255,215,0,0.35);position:relative;overflow:hidden;}
...
<div class="container">
  ...
  <div class="pay-detail" id="detail-esewa">
    ...
    <div class="pay-row"><span>eSewa ID</span><span>9800000000 <button class="copy-btn" id="copyEsewa">Copy</button></span></div>
  ...
  <div class="pay-detail" id="detail-bank">
    ...
    <div class="pay-row"><span>Bank</span><span>NIC Asia Bank</span></div>
    <div class="pay-row"><span>Account No.</span><span>1234567890123456 <button class="copy-btn" id="copyBank">Copy</button></span></div>
    <div class="pay-row"><span>Account Name</span><span>MLBB TopUp Nepal</span></div>
  ...
  <div class="contact-section" style="margin-top:40px;">
    ...
    <div class="contact-btns">
      <a href="https://wa.me/977XXXXXXXXXX" class="contact-btn btn-whatsapp" target="_blank">&#x1F4F1; WhatsApp Us</a>
      <a href="https://t.me/yourtelegram" class="contact-btn btn-telegram" target="_blank">&#x2708;&#xFE0F; Telegram</a>
    </div>
  </div>
...
function loadPackages() {
  ...
  .then(function(pkgs){
    ...
    pkgs.forEach(function(p, i) {
      ...
      card.innerHTML =
        ...
        '<div class="price-new">Rs' + p.price + '</div>' +
        '<div class="discount-tag">BEST</div>' +
        '</div>';
      card.addEventListener('click', function(){ selectPackage(p); });
      grid.appendChild(card);
    });

    // Also fill price table
  ...
  .catch(function(){
    document.getElementById('diamondGrid').innerHTML = '<div class="loading-grid">Failed to load packages. Is the backend running?</div>';
  });
}

function selectPackage(p) {
  if (selectedPackage) {
    var old = document.getElementById('dc-' + selectedPackage.id);
...
function placeOrder() {
  ...
  var receiptInput = selectedPay === 'esewa'
    ? document.getElementById('receiptEsewa')
...
document.getElementById('copyEsewa').addEventListener('click', function(){ copyText('9800000000'); });
document.getElementById('copyBank').addEventListener('click',  function(){ copyText('1234567890123456'); });
...
# ── Also add CORS fix to backend if needed ──────────────────
Write-Host "[2/2] Checking backend CORS for file:// access..." -ForegroundColor Yellow

$serverPath = Join-Path (Get-Location).Path "backend\server.js"
$serverContent = Get-Content $serverPath -Raw

if ($serverContent -notmatch "origin.*true") {
    $serverContent = $serverContent -replace "app\.use\(cors\(\)\)", "app.use(cors({ origin: true, credentials: true }))"
    [System.IO.File]::WriteAllText($serverPath, $serverContent, [System.Text.Encoding]::UTF8)
    Write-Host "      CORS updated to allow all origins (dev mode)." -ForegroundColor Green
} else {
...
Write-Host "  - D:\MLBB\mlbb-topup.html   (wired to API)" -ForegroundColor Gray
Write-Host "  - D:\MLBB\backend\server.js (CORS fixed)" -ForegroundColor Gray
Write-Host ""
Write-Host "  WHAT CHANGED:" -ForegroundColor Cyan
Write-Host "  - Diamond cards now load LIVE from your database" -ForegroundColor Gray
Write-Host "  - Place Order submits real data to backend" -ForegroundColor Gray
Write-Host "  - Receipt image upload included in order" -ForegroundColor Gray
Write-Host "  - Price table loads from DB (always up to date)" -ForegroundColor Gray
Write-Host "  - Success banner shows order reference number" -ForegroundColor Gray
Write-Host "  - Admin panel will instantly see new orders" -ForegroundColor Gray
Write-Host ""
Write-Host "  HOW TO TEST:" -ForegroundColor Cyan
Write-Host "  1. Restart backend:  cd backend && npm run dev" -ForegroundColor Gray
Write-Host "  2. Open:  D:\MLBB\mlbb-topup.html" -ForegroundColor Gray
Write-Host "  3. Fill in a Game ID, pick a package, pick payment" -ForegroundColor Gray
Write-Host "  4. Click Place Order" -ForegroundColor Gray
Write-Host "  5. Open admin panel - the order appears instantly" -ForegroundColor Gray
Write-Host ""
Write-Host "  READY FOR PHASE 5?" -ForegroundColor Cyan
Write-Host "  Phase 5 = Docker + nginx + deploy to VPS" -ForegroundColor Gray
Write-Host ""
